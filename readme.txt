...
= 1.13.17 =
* Fix: `dataLayer is not defined` ReferenceError when third-party tags (gtm4wp, Rank Math, similar) emit a bare `dataLayer.push()` before the GTM bootstrap is allowed to run. The consent banner now inserts `window.dataLayer = window.dataLayer || []` as a `before` inline on its own handle, so it executes ahead of the main script tag and any later inline tag on the page. Queued events are picked up normally once consent is granted and GTM initialises. Resolves wp.org/support/topic/bug-report-datalayer-is-not-defined.
* Fix: cookie category counters in the admin sidebar no longer stay at their pre-scan values after a scan plus auto-categorisation. The full cookie-lifecycle (create, update, delete) now invalidates every consumer that reads the per-category cookie list — Category and Cookie controller caches, banner template option cache, IAB unmatched-vendors transient, and the page-cache adapters for LiteSpeed, WP Rocket, W3 Total Cache, WP Fastest Cache, WP Super Cache, Autoptimize, Hummingbird, Breeze, Cache Enabler, SiteGround Optimize. Resolves wp.org/support/topic/bug-report-cookie-categories-not-populated.
* Fix: the asymmetric REST `bulk_update` handler no longer drops `opt_in_script` / `opt_out_script` fields silently. Refactored to iterate the schema's editable properties so every field declared in `get_item_schema()` round-trips through the same `sanitize_script_field` capability gate as single-cookie updates. Adds a regression spec.
* Fix: `_cookieScripts` payload no longer silently truncates at 500 cookies. Replaced the `LIKE %script%` query with a paged loop (ORDER BY cookie_id ASC, 1000-row pages, 10000-row hard ceiling with an `error_log` warning when reached). The LIKE pattern is now anchored on the JSON key shape (`"opt_in_script":"`) so descriptions that happen to mention the field name no longer false-positive.
* Fix: `sanitize_meta_for_current_user` helper now intercepts every write path into `wp_faz_cookies.meta` (REST PUT, REST `bulk_update`, settings import for cookies and categories, internal `Cookie::set_meta`, WP-CLI `wp faz import`). Closes a stored-XSS surface where a multisite Site Administrator with `manage_options` but without `unfiltered_html` could plant arbitrary JavaScript into the opt-in/opt-out script field by uploading a tampered settings export. `unfiltered_html` is now required at every layer.
* Feature: `Necessary` is now selectable in the Custom Blocking Rules dropdown (Cookies admin → Custom Blocking Rules table). Admins can permanently exempt always-on scripts (Cloudflare Turnstile, Google reCAPTCHA, hCaptcha, Wordfence, GTM if you treat it as required, etc.) from blocking and the scanner respects the rule across re-scans. The settings sanitiser already accepted `necessary`; only the picker was missing it. Resolves wp.org/support/topic/feature-request-add-necessary-category-to-script-blocker.
* UX: `Enable Cookie banner` toggle now also appears at the top of the Cookie Banner page (General tab, above Design Presets) as a `Banner Status` card. It mirrors the existing entry under Settings → Banner Control — both write to the same option so they cannot drift. The Settings location stays in place for muscle memory.
* UX: removing the Escape-key-hides-banner behaviour on the consent banner — visitors can no longer accidentally dismiss the banner without recording a consent decision (EDPB dark-pattern concern). Escape continues to close the preference center modal (standard a11y behaviour).
* UX (a11y): DSAR submit and CCPA opt-out submit buttons now expose `aria-busy` and a localised submitting label during the network round-trip. Validation failures on the DSAR form now mark the offending input with `aria-invalid`, set per-field `aria-describedby` error messages, focus and scroll the first invalid field, and announce the error via `role=alert` / `aria-live=assertive`. Success notices keep `role=status` / `aria-live=polite`. Required DSAR inputs carry `aria-required` even though the form uses `novalidate`. `.faz-dsar-btn` and `.faz-dnsmpi-btn` now expose a contrasting focus indicator (white outline, dark box-shadow against the blue button) satisfying WCAG 1.4.11.
* Compliance: CCPA Section 1798.135(c) — the `[faz_do_not_sell]` shortcode now renders a "Withdraw opt-out" button alongside the success notice for visitors who already opted out. Clicking it clears the opt-out cookie and writes a `dns_rescinded` row to `wp_faz_consent_logs`. Visitors who change their mind no longer need to delete cookies manually.
* Fix: admin `Edit` button on the Cookies table now shows a spinner during the GET round-trip (via the existing `FAZ.btnLoading` helper) so slow networks no longer surface as a frozen UI.
* Fix: the script-execution disclaimer beneath the opt-in/opt-out script textareas (Cookies admin → script field) is now rendered as a visible warning callout that explicitly mentions CSP, secrets, and runtime execution scope. The previous 11px grey paragraph understated the security impact of the field.
* Fix: `consentExpiry` UI fallback in the banner admin now defaults to 180 days, matching the JSON config defaults and the Garante Privacy / EDPB six-month guidance. Prior fallback was 365 days for installs that opened the banner editor before saving for the first time.
* Internal: cookie scanner imports, banner template regeneration, IAB GVL coverage checks, page-cache plugin purges, and the script-blocking auto-discovery pipeline all now respond to `faz_after_create_cookie` and `faz_after_delete_cookie` in addition to the legacy `faz_after_update_cookie`. Listeners are symmetrical and locked in by a regression spec.
* Internal: documentation fixes on `wp_localize_script` ordering vs `wp_add_inline_script('before')` (the localised blob is emitted in the `data` bucket which renders before `before` inlines), JSDoc `@returns` on `_fazAcceptCookies`, `phpcs:disable` directive coverage on the new `cookieScripts` SELECT, `get_default()` docblock alignment with the de-branded return shape, and a corrected `manage_options` → `unfiltered_html` reference in `_fazRunScript`.
