# Notice at Collection & Política de Cookies (CCPA/CPRA)

**Última atualização:** {{LAST_UPDATED_DATE}}

Esta Notice e Política de Cookies descreve como **{{COMPANY_NAME}}** coleta, usa e compartilha Informações Pessoais por meio deste site ({{COOKIE_POLICY_URL}}), em conformidade com o California Consumer Privacy Act de 2018, conforme alterado pelo California Privacy Rights Act (CCPA/CPRA, Cal. Civ. Code §§ 1798.100 e seguintes).

## Contato

- **Empresa:** {{COMPANY_NAME}} — {{COMPANY_ADDRESS}} — {{COMPANY_EMAIL}}
- **Registro:** {{COMPANY_REGISTRY}}

## Categorias de Informações Pessoais coletadas (§ 1798.140(v))

- **Identificadores** — IP (hash), identificadores de dispositivo/online
- **Atividade em redes eletrônicas** — histórico de navegação, interações
- **Geolocalização aproximada** — país/região via IP
- **Inferências** — para análises

NÃO coletamos Informações Pessoais Sensíveis sem opt-in explícito, nem dados de menores de 16 anos sem opt-in afirmativo.

## Cookies utilizados

{{COOKIE_CATEGORIES}}

## Destinatários

{{THIRD_PARTY_SERVICES}}

**Não vendemos nem compartilhamos** Informações Pessoais para publicidade comportamental entre contextos sem opt-in. Para opt-out, clique em "Do Not Sell or Share" no rodapé.

## Retenção

{{RETENTION_PERIOD}} a partir da coleta.

## Direitos CCPA/CPRA

Direito de saber, eliminar, corrigir, optar pela exclusão da venda/compartilhamento, limitar o uso de Informações Pessoais Sensíveis, não discriminação, portabilidade. E-mail **{{COMPANY_EMAIL}}** — resposta em 45 dias.

## Global Privacy Control

Honramos o sinal GPC como opt-out válido.

## Autoridade

California Privacy Protection Agency: {{CA_PIPC_CONTACT}}, [{{OFFICIAL_RESOURCES_URL}}]({{OFFICIAL_RESOURCES_URL}})
